...
export const projectPost = async (req, res) => {
  console.log("reques comes for post project")
  ...
  upload.single("image")(req, res, async (err) => {
    ...
    const { title, description, technology,category } = req.body;
    ...
    const project = new Project({ title, description, image: req.file.path, technology,category });
...
export const projectGet = async (req, res) => {
        try {
            const project=await Project.find({ category: "web" });
            //console.log(project)
...
export const MLprojectGet = async (req, res) => {
        try {
            const project=await Project.find({ category: "ml" });
            //console.log(project)
            return res.status(200).json(project)
        } catch (error) {
                return res.status(500).send("Error fetching projects");
